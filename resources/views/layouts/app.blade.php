            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item">
                    <form id="language-form" action="{{ route('setLocale') }}" method="POST">
                        @csrf
                        <select name="locale" onchange="document.getElementById('language-form').submit();" class="form-control">
                            <option value="en" {{ session('locale', 'en') == 'en' ? 'selected' : '' }}>English</option>
                            <option value="ar" {{ session('locale', 'en') == 'ar' ? 'selected' : '' }}>Arabic</option>
                        </select>
                    </form>
                </li>
            ...
            <ul class="navbar-nav ml-auto">
                ...
                <li class="nav-item dropdown user-menu">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">
                        <img src="{{ Storage::url(Auth::guard('web')->user()->image) }}" class="user-image img-circle elevation-2" alt="User Image">
                        <span class="d-none d-md-inline">{{ Auth::guard('web')->user()->name ?? 'Dashboard' }}</span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                        <!-- User image -->
                        <li class="user-header bg-cyan">
                            <img src="{{ Storage::url(Auth::guard('web')->user()->image) }}" class="img-circle elevation-2" alt="User Image">
                            <p>
                                {{ Auth::guard('web')->user()->name ?? 'Dashboard' }}
                                <small>Member since {{ Auth::guard('web')->user()->created_at->format('M Y') }}</small>
                            </p>
                        </li>
                        <!-- Menu Body -->
                        <li class="user-body">
